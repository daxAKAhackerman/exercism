...
int meetup_day_of_month(unsigned int year, unsigned int month, const char *week,
  ...
  // Get first of month
  struct tm fotm = {.tm_year = year - 1900,
                    .tm_mon = month - 1,
                    .tm_mday = 1,
                    .tm_isdst = -1};
  time_t first_of_the_month = mktime(&fotm);
  localtime_ptr = localtime(&first_of_the_month);
  struct tm normalized_fotm = *localtime_ptr;

  // And the last of the month
  struct tm lotm = {
      .tm_year = year - 1900, .tm_mon = month, .tm_mday = 0, .tm_isdst = -1};
  time_t last_of_the_month = mktime(&lotm);
  localtime_ptr = localtime(&last_of_the_month);
  struct tm normalized_lotm = *localtime_ptr;
